...
export function HabitDay ({defaultCompleted = 0, amount = 0, date}: HabitDayProps) {
  ...
  const isToday = dayjs(date).isSame(currentDay)

  function handleCompletedeChanged(completed: number){
    ...
    <Popover.Root>
      ...
      <Popover.Portal>
        ...
        <Popover.Content className='min-w-[320px] p-6 rounded-2xl bg-zinc-900 flex flex-col'>
          ...
          <HabitsList
            date={date}
            onCompletedChanged={handleCompletedeChanged}
          />
